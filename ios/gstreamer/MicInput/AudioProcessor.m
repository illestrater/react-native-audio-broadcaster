...
-(void)start:(NSString *)url withPort:(NSNumber *)port withPassword:(NSString *)password withMount:(NSString *)mount
{
    shout_init();

    NSLog(@"STARTING BROADCAST");
    NSLog(@"URL%@", url);
    NSString *portValue = port;
    NSLog(@"PORT%@", portValue);
    NSLog(@"PW%@", password);
    NSLog(@"MOUNT%@", mount);

    const char *cUrl = [url UTF8String];
    const char *cPassword = [password UTF8String];
    const char *cMount = [mount UTF8String];
	...
	if (shout_set_host(shout, cUrl) != SHOUTERR_SUCCESS) {
	...
	if (shout_set_port(shout, 1337) != SHOUTERR_SUCCESS) {
		printf("Error setting port: %s\n", shout_get_error(shout));
		return;
	}

	if (shout_set_password(shout, cPassword) != SHOUTERR_SUCCESS) {
		printf("Error setting password: %s\n", shout_get_error(shout));
		return;
	}
	if (shout_set_mount(shout, cMount) != SHOUTERR_SUCCESS) {
    ...
    // ShoutOutputStream_Init("cue.dj", 1337, "holyshit", "source", "ethsavedmusic", 1);
    mLame = lame_init();
